# This Makefile is for the HTTP::Client extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/HTTP/Client.pm]
#     AUTHOR => q[Nightcat <nightcat@crocker.com>]
#     NAME => q[HTTP::Client]
#     PREREQ_PM => { HTTP::Lite=>q[0] }
#     VERSION_FROM => q[lib/HTTP/Client.pm]
...
PARENT_NAME = HTTP
DLBASE = $(BASEEXT)
VERSION_FROM = lib/HTTP/Client.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/HTTP/Client.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DIRFILESEP)Config.pm $(PERL_INC)$(DIRFILESEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/HTTP
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/HTTP
...
DISTNAME = HTTP-Client
DISTVNAME = HTTP-Client-1.43
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  lib/HTTP/Client.pm $(INST_MAN3DIR)/HTTP::Client.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  blib/lib/.DS_Store blib/lib/HTTP/Client.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         HTTP-Client' >> META.yml
	$(NOECHO) $(ECHO) 'version:      1.43' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: lib/HTTP/Client.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    HTTP::Lite:                    0' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Class for creating HTTP clients.</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Nightcat &lt;nightcat@crocker.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTTP-Lite" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/HTTP/Client.pm blib/lib/HTTP/Client.pm \
